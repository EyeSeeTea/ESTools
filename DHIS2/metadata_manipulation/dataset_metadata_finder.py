...
def main():
    ...
    rows = list()
    dataset = d2.get(dataset_query % args.dataset)
    ...
    with open(args.output, mode='w') as file:
        ...
        for row in rows:
            for category_option_combo in row["category_combo_options"]:
                print([dataset["name"], row["name"], row["id"], row["code"], row["topic"],
                                         category_option_combo["name"], category_option_combo["id"],
                                         "Xr12mI7VPn3", ""])
                csv_final_file.writerow([dataset["name"], row["name"], row["id"], row["code"], row["topic"],
                                         category_option_combo["name"], category_option_combo["id"],
                                         "Xr12mI7VPn3", ""])
