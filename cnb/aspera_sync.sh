...
syncPath(){
        ...
        result=$(aspera shares browse -i -u $REMOTE_USER --host $REMOTE_HOST \
         --path=$REMOTE_FOLDER --sort mtime_a |
        sed -n  '/----/,/----/p' |sed '$d;1d;2d' )
        ...
        while IFS= read -r asperarow;do
                ...
                if ! [ -z "$lsfilerow" ];then
                        ...
                        localfiledate=$(busybox date -u -d $localdate -D \
                         %Y-%m-%d%H:%M:%S)
                ...
                if [[ "$localfilenames" == *"$filename"* ]] && \
                [[ $filedate -lt $localfiledate ]];then
